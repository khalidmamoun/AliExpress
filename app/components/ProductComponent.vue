    ...
    >
      ...
      <div class="p-4 flex flex-col flex-1 ">
        ...
        <div class="flex flex-wrap gap-1 mt-2">
          ...
          <span class="text-xs bg-gray-100 text-gray-700 px-2 py-0.5 rounded">{{ product.sold }}+ sold</span>
...
// السعر القديم (5% زيادة كمثال)
const oldPriceFormatted = computed(() => {
  if(!product.value.price) return null
  let oldPrice = product.value.price + product.value.price / 20
  return oldPrice.toLocaleString()
})

// عدد المبيعات (افتراضي لو مش موجود)
if(!product.value.sold) product.value.sold = 5000
</script>
